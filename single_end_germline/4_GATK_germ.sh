...
filesfolder=$analysis_dir/3_BAM_files/3.2_deduppedBamSortedFiles
ploidy=2 
...
mkdir $analysis_dir/4_processed_BAM/4.3_BP_RNA_files/ -p
...
for i in $files; do
    ...
    # Expected Result
    #This creates a file called recal_reads.bam containing all the original reads, but now with accurate base substitution, insertion and deletion quality scores. By default, the original quality scores are discarded in order to keep the file size down. However, you have the option to retain them by adding the flag –emit_original_quals to the PrintReads command, in which case the original qualities will also be written in the file, tagged OQ.

    ############################################  4.3 Variant calling (HaplotypeCaller) #####################################

    #https://gatkforums.broadinstitute.org/gatk/discussion/4535/any-ploidy-goes


    echo 'Step 4.3. HaplotypeCaller:' $sample_name
    
    java $memory -jar $softwares_GATK HaplotypeCaller \
    -R $reference_genome38 \
    -I   $analysis_dir/4_processed_BAM/4.2_recal_reads/$sample_name.bam \
    -ploidy $ploidy \
    --emit-ref-confidence GVCF --dont-use-soft-clipped-bases True \
    -O $analysis_dir/4_processed_BAM/4.3_BP_RNA_files/$sample_name.g.vcf
    --tmp-dir $temp_folder
                ...
                ########################################################################################################################
                                      ...
                                      ############### End  ###############
## the final ouput is stored in  $analysis_dir/4_processed_BAM/4.3_BP_RNA_files/*.g.vcf
